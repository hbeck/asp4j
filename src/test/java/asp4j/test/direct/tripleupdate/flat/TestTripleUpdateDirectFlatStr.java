...
/**
 * Triple Update. Direct mapping via interfaces, not terms. Only strings for values.
 *
 * @author hbeck date 2013-05-14
 */
public class TestTripleUpdateDirectFlatStr {
    ...
    public void test(Solver externalSolver, String rulefile) throws Exception {
        ...
        cautiousConsequence = solver.getConsequence(program, ReasoningMode.CAUTIOUS);
        braveConsequence = solver.getConsequence(program, ReasoningMode.BRAVE);
        assertTrue(CollectionUtils.isEqualCollection(cautiousConsequence, braveConsequence));
    ...
    public void test_conflict(Solver externalSolver, String rulefile) throws Exception {
        ...
        Program<CanAsAtom> program = new ProgramBuilder<CanAsAtom>()
                .add(new File(rulefile))
